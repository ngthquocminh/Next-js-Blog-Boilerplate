...
const handleFormInputAsync = async (name:string,phone:string,email:string) => {
  // const files = fs.readdirSync(postsDirectory);
  await new Promise(resolve => setTimeout(resolve, 3000));
  const dateStr = new Date().toISOString();
  const createdFile = stringToSlug(name+'-'+phone+'-'+dateStr)+'.json'
  fs.writeFileSync(join(bookingsDirectory,createdFile),JSON.stringify({name,phone,email}))
}